...
- **✅ Code-Stand Ei (22.07 GELÖST):** Das Löcher-Problem des **Ei-Modells** ist gelöst — Ei lückenlos (0 % unbedeckt), Welle über das ganze Ei (3000/3000), Aktivierung bei Timestep 220. **Kern-Erkenntnis: die Ursache war das Zell-Sampling, nicht die Kraft/Radius-Logik.** Fix = flächengleicher Cell-Input `dr1p75_0.vtk` (statt der rohen, bauchdünnen `horizontal_egg`) + per-Zelle-Radien + Render-Überlappungsfeld `render_radius`. Kraft `force_type='l'`, `d_r_e=0`, weiches Pinning 3, Relax als eigener `relax`-Modus → `egg_cells_relaxed.vtk`. Relaxierung kann eine ungleichmäßige Datei nachweislich nicht egalisieren. Vollständig → Ist-Box unten. §13 + 09.–15.07-Historie bleiben als Verlauf. Der **Kappen-/Sheet-Teil (Exp. 1)** ist unberührt.

---

## ✅ Aktueller Ei-Stand (22.07.2026, GELÖST) — überschreibt §1/§2/§11 für MODEL_EGG

> **Löcher-Problem gelöst.** Das Ei ist **lückenlos** (0 % unbedeckt), die Welle läuft über das **ganze** Ei (3000/3000 Zellen feuern), Aktivierung bei Timestep 220. **Die Ursache war die Zell-Sampling-Datei — NICHT die Kraft-/Radius-Logik, nicht das Rendering, nicht die Relaxierung.** §13 + die alte 15.07-Box bleiben historisch. **Betrifft nur MODEL_EGG** — MODEL_CAP/MODEL_SHEET (Exp. 1) laufen unverändert mit §1/§2.

- **KERN-URSACHE = flächengleiches Sampling der Startzellen.** Der rohe `horizontal_egg_tribolium_1.vtk` wird von `create_egg.py` zufällig über die *Surface-Punkte* gezogen → uniform pro **Punktzahl**, erbt damit das pol-dichte Surface-Sampling → **dünner Bauch** (Bauch-Abstand 1.13 vs. Pol 0.88, corr(Pol-Nähe, Abstand) = −0.40). Joerns `dr1p75_0.vtk` ist uniform pro **Fläche** (Bauch = Pol = 1.11, corr −0.09). **Cell-Input jetzt `Joern_Projekt/initial_data/dr1p75_0.vtk`** (`raw_cells` in foundation.cu). Pinning-Surface weiter 33164 aus `surfaces/target_surface_tribolium_1.vtk`.
- **Relaxierung kann eine ungleichmäßige Datei NICHT egalisieren (bewiesen):** selbst starkes konstantes Rauschen + Selbstkalibrierung + weiches Pinning über 1500 Steps → corr −0.49 (nie −0.09). Grund: auf variabel gekrümmter, geschlossener Fläche mit Adhäsion ist das Packungs-Gleichgewicht **intrinsisch ungleichmäßig** (reine Abstoßung sogar −0.86). Gleichmäßigkeit muss beim **Erzeugen** der Zellen entstehen (flächengleiches Sampling), nicht per Relaxierung.
- **Was NICHT die Ursache war (getestet, nicht weiter suchen):** Kraftgesetz (Morse ≈ gedeckt-linear ≈ Joerns Zentrumsabstand-Kraft — bit-identische Welle); Radius- vs. Zielabstand-Logik (mathematisch identisch); Rendering (point_size vs. Kugeln).
- **Kraft = gedeckte lineare Kraft** `force_type='l'` fürs Ei (Repulsion/Adhäsion, keine Fern-Kohäsion). Sheet/Cap Morse `'m'`. `d_r_e=0` fürs Ei (Gleichgewicht = Kontakt), Cutoff `egg_r_cut=1.5` (~6 Nachbarn), Grid-`cube_size=r_max=2`.
- **Per-Zelle-Radien:** jede Zelle `r_start_i = ½ · mittlerer 6-Nachbar-Abstand` (aus den relaxierten Positionen beim Wellenstart berechnet, Helper `compute_cell_radii`); `r_e=1.111`. Schrumpfen/Wachsen relativ zum **eigenen** Radius (`egg_shrink_factor=0.5`, State-Machine nutzt `pc_r_start`/`pc_r_activated`). Spannungsfrei (r_i+r_j ≈ lokaler Abstand).
- **Render-Radius (nur Darstellung, ändert die Physik NICHT):** Extra-VTK-Feld `render_radius = egg_render_overlap(1.4) · Physik-Radius`. **In Vedo nach `render_radius` skalieren/einfärben → 0 % unbedeckt.** Grund: sich gerade berührende Kreise/Kugeln decken nur π/(2√3)=90.7 % (Zwischenräume bleiben immer); Überlappung füllt sie. Feld `radius` bleibt der echte Physik-Radius. Höhere Überlappung: 1.6 → 3.2 %, 1.8 → 2.0 % unbedeckt.
- **Weiches Pinning** `surface_stiffness=3` fürs Ei (Joerns Wert; Zellen gleiten frei, mehr sichtbare Bewegung, bleiben auf der Fläche — max. Off-Surface ~0.30 = Mesh-Auflösung, kein Drift). Sheet/Cap 12.
- **Relax-Pipeline:** Modus `relax` lädt die Zellen, settlet `egg_relax_steps=500` Steps (nur Kräfte + Pinning), schreibt `egg_cells_relaxed.vtk`, beendet. Wellenlauf lädt diese Datei. Kein Frame-Offset, Aktivierung bei Timestep 220.
- **Bewegung ist Jamming-limitiert (korrekt, kein Bug):** Nachbarn kontrahierender Zellen rücken nur ~0.02 nach (58 % vs. 50 % zufällig), weil die starken Paarkräfte im dichten Verband isotrop **auslöschen** (Netto-Kraft-Median ~0.01). Eine Kontraktionswelle in einem konfluenten Epithel ist eine Welle der *Kontraktion*, nicht der Translation.
- **Entfernt (22.07, Sackgassen — NICHT wieder einbauen):** `force_type='j'`-Zweig (Joerns Zentrumsabstand-Logik, war identisch zu 'l'); Selbstkalibrierung (`mean_nn_spacing`) + Brownsche Bewegung in der Relaxierung (egalisiert nachweislich nicht).
- **🟡 Für EIGENE Eier (freie Dichte, unabhängig von Joerns Dateien):** flächengleicher Sampler nötig (Farthest-Point / Poisson-Disk auf `target_surface`) — angeboten, noch nicht gebaut. Ansonsten gelöst.
